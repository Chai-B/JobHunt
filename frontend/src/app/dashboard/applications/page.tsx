...
export default function ApplicationsPage() {
    ...
    const syncInbox = async () => {
        setSyncing(true);
        const loadingToast = toast.loading("Scanning inbox for job emails...");
        try {
            const token = localStorage.getItem("token");
            ...
            if (res.ok) {
                toast.success("Inbox scan complete — applications updated.", { id: loadingToast });
                // Delay fetch to let async processing finish
                setTimeout(() => fetchApplications(), 2000);
            } else {
                const err = await res.json().catch(() => ({}));
                toast.error(err.detail || "Sync failed.", { id: loadingToast });
            }
        } catch (err) {
            toast.error("Network error during inbox sync.", { id: loadingToast });
        } finally {
            setSyncing(false);
        }
    };
    ...
    const onUpdateStatus = async (id: number, status: string) => {
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`${API_BASE_URL}/api/v1/applications/${id}/status`, {
                method: "PUT",
                headers: {
    ...
    const getStatusBadge = (status: string) => {
        ...
        switch (status?.toLowerCase()) {
            ...
            case "offer_received": return <Badge className={`${base} bg-emerald-500/10 text-emerald-500 border-emerald-500/20`}>Offer Received</Badge>;
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <Button size="lg" onClick={syncInbox} disabled={syncing} className="gap-3 bg-primary text-primary-foreground hover:opacity-90 rounded-2xl px-8 shadow-xl shadow-primary/20 transition-all duration-300">
                        <RefreshCw className={`w-4 h-4 ${syncing ? 'animate-spin' : ''}`} /> {syncing ? 'Scanning...' : 'Sync Now'}
                    </Button>
                </div>
            </div>
                                            ...
                                            filteredApplications.map((app) => (
                                                <React.Fragment key={app.id}>
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell className="font-semibold">{app.company_name || 'Unknown'}</TableCell>
                                                        <TableCell className="text-sm text-foreground/80">{app.job_title || app.contact_role || "General Application"}</TableCell>
                                                        ...
                                                        <TableCell className="text-right px-8">
                                                            <div className="flex items-center justify-end gap-1">
                                                                <Button variant="ghost" size="icon" className="rounded-full h-8 w-8 text-muted-foreground hover:text-destructive" onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(app.id); }}>
                                                                            ...
                                                                            <div className="space-y-10">
                                                                                <div className="space-y-4">
                                                                                    ...
                                                                                    <div className="mt-8 border-l border-border/50 ml-1.5">
                                                                                        ...
                                                                                        {!(app.notes?.match(/\[([^\]]+)\] ([^:]+): (.+?)  -> \(Status: ([^)]+)\) \[ID: ([^\]]+)\]/g))?.length && (
                                                                                            <div className="pl-8 py-4 text-xs text-muted-foreground/50">No interaction timeline yet. Sync your inbox to populate.</div>
                                                                                        )}
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                ...
                <AlertDialogContent className="bg-card border-border/50 shadow-2xl rounded-2xl max-w-sm">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-muted-foreground">
                            Are you sure? This will permanently remove this application and its timeline history.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="gap-2">
                        <AlertDialogCancel className="rounded-xl border-border/50">Cancel</AlertDialogCancel>
                        <AlertDialogAction
                            onClick={() => deleteConfirmId && deleteApplication(deleteConfirmId)}
